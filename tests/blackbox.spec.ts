...
const apiToken = Effect.runSync(
  Config.string("TEST_API_TOKEN")
    .pipe(Config.withDefault("local-development-only"))
...
function parseBlackboxJson(text: string | null): unknown {
  if (text === null) {
    throw new Error("Blackbox response text is missing");
  }
...
test("frontend health and security headers", async ({ request }) => {
  ...
  expect(healthBody).toEqual(
    expect.objectContaining({ status: "ok", environment: expect.any(String) }),
  );
...
test("real note storage and workflow produce uppercase content through the frontend", async ({
  request,
}) => {
  ...
  expect(storedBody).toEqual(expect.objectContaining({ id: noteId, content }));

  const started = await request.post("/api/jobs", {
    headers: authenticatedHeaders,
    data: { noteId },
  });
  ...
  await expect
    .poll(
      ...
      },
      {
        ...
        message: "The real workflow must persist its completed uppercase result",
      },
    )
    .toEqual(
...
test("all authenticated routes reject missing and incorrect bearer credentials", async ({
  request,
}) => {
  const noteId = randomUUID();

  await Promise.all(
    [{}, { Authorization: `Bearer invalid-${randomUUID()}` }].map(async (headers) => {
      await Promise.all([
        expectHttpStatus(request.get(`/api/notes/${noteId}`, { headers }), 401),
        expectHttpStatus(request.get(`/api/jobs/${randomUUID()}`, { headers }), 401),
        expectHttpStatus(
          request.post("/api/notes", { headers, data: { content: "Unauthorized note" } }),
          401,
        ),
...
test("invalid note and job inputs fail at the real API boundary", async ({ request }) => {
  ...
  await Promise.all(
    [{}, { noteId: "" }, { noteId: 42 }, { noteId: "not-a-uuid" }].map((data) =>
      expectHttpStatus(request.post("/api/jobs", { headers: authenticatedHeaders, data }), 400),
    ),
  );

  const malformed = await request.post("/api/notes", {
  ...
  const wrongMedia = await request.post("/api/notes", {
  ...
  await expectHttpStatus(
    request.get(`/api/notes/${missingNote}`, { headers: authenticatedHeaders }),
    404,
  );

  await expectHttpStatus(
    request.post("/api/jobs", { headers: authenticatedHeaders, data: { noteId: missingNote } }),
    404,
  );
});

test("developer UI creates and reads a note, completes a workflow, and clears the token on reload", async ({
  page,
}) => {
  await page.goto("/");
  ...
  const createdNoteText = await page.locator("#note-result").textContent();

  expect(parseBlackboxJson(createdNoteText)).toEqual(
    expect.objectContaining({ id: noteId, content }),
  );
  ...
  const readResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname === `/api/notes/${noteId}` &&
  ...
  const storedNoteText = await page.locator("#note-result").textContent();

  expect(parseBlackboxJson(storedNoteText)).toEqual(
    expect.objectContaining({ id: noteId, content }),
  );
    ...
    .poll(
      async () => {
        const statusResponse = page.waitForResponse(
          (response) =>
            new URL(response.url()).pathname === `/api/jobs/${jobId}` &&
            response.request().method() === "GET",
        );
        ...
        const jobText = await page.locator("#job-result").textContent();
        return parseBlackboxJson(jobText);
      },
  ...
  const storageCounts = await page.evaluate(() => [localStorage.length, sessionStorage.length]);

  expect(storageCounts).toEqual([0, 0]);
  await page.reload();
