...
function resolveWorkflowDefinition() {
  ...
  if (isSourceEntrypoint) {
    return { workflowsPath: fileURLToPath(new URL("temporal_workflow.ts", import.meta.url)) };
  }

  return {
    workflowBundle: {
      codePath: fileURLToPath(new URL("temporal_workflow_bundle.js", import.meta.url)),
    },
  };
}
