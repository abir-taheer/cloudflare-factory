...
test(
  ...
  async () => {
    ...
    try {
      ...
      const clientRequests = peer.authenticatedRequests();

      assert.ok(clientRequests > 0);

      const worker = await NativeConnection.connect(options);

      await worker.close();

      const authenticatedRequests = peer.authenticatedRequests();

      assert.ok(authenticatedRequests > clientRequests);
      ...
      assert.equal(peer.authenticatedRequests(), authenticatedRequests);
    } finally {
      await peer.close();
    }
  },
