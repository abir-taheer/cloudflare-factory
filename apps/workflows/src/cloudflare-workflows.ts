...
import { cloudflarePlatformLayer, DurableObjectLeaseStorage, type CloudflarePlatformBindings } from '@factory/platform/cloudflare';
...
export class DemoWorkflow extends WorkflowEntrypoint<CloudflarePlatformBindings, BackgroundJob> {
  override async run(event: WorkflowEvent<BackgroundJob>, step: WorkflowStep) {
    return step.do('process-note', {retries:{limit:5,delay:'2 seconds',backoff:'exponential'}}, () => Effect.runPromise(runNoteJob(event.payload).pipe(Effect.provide(cloudflarePlatformLayer(this.env)))));
  }
}
...
export class JobCoordinator extends DurableObject {
  ...
  acquire(token: string, ttlMs: number): Promise<boolean> { return this.leases.acquire(token,ttlMs); }
  release(token: string): Promise<boolean> { return this.leases.release(token); }
}
export default {
  ...
  queue(batch: MessageBatch<BackgroundJob>, bindings: CloudflarePlatformBindings): Promise<void> {
    return Effect.runPromise(Effect.forEach(batch.messages, (message) =>
      Effect.tryPromise(() => bindings.WORKFLOW.create({id:message.body.id,params:message.body})).pipe(
        // A duplicate is accepted only after a successful durable read-back.
        Effect.matchEffect({onSuccess: Effect.succeed, onFailure: () => Effect.gen(function* () {
          const workflow = yield* Effect.tryPromise(() => bindings.WORKFLOW.get(message.body.id));
          return yield* Effect.tryPromise(() => workflow.status());
        })}),
        Effect.match({onFailure: () => { message.retry(); }, onSuccess: () => { message.ack(); }}),
