...
cleanup() {
  rm -f "$DOPPLER_DEV_DIRECTORY"/{api,frontend,workflows}.env
  rm -f "$DOPPLER_DEV_DIRECTORY"/{api,frontend,workflows}.json
...
docker compose -f compose.yaml run --rm --no-deps -T tools \
  npx tsx scripts/development/development_config_cli.ts "$DOPPLER_DEV_DIRECTORY" \
  "$DOPPLER_API_PROJECT" "$DOPPLER_FRONTEND_PROJECT" \
  "$DOPPLER_WORKFLOWS_PROJECT"
