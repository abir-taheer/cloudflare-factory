...
import { previewGithubRequest } from "../preview/preview-github.ts";
import { previewIo, previewRecord, previewString } from "../preview/preview-model.ts";
import { ProductionAppSchema, parseProductionValue, productionPrefix } from "./production-model.ts";
...
/** Verify immutable repository identity and current main head before mutations and final sign-off. */
export const verifyProductionRevision = (dispatch: ProductionDispatch) =>
  Effect.gen(function* () {
    const repository = yield* previewGithubRequest(`/repos/${dispatch.REPOSITORY}`);
    const head = yield* previewGithubRequest(`/repos/${dispatch.REPOSITORY}/commits/main`);

    if (
      String(repository["id"]) !== dispatch.REPOSITORY_ID ||
      repository["default_branch"] !== "main" ||
      head["sha"] !== dispatch.REVISION
    ) {
      throw new Error("Production main revision changed");
    }
  });
