...
export const readPreviewPullRequest = (repository: string, pr: number) =>
  Effect.gen(function* () {
    if (
    ...
    if (
      ...
      headRepository["id"] !== baseRepository["id"]
    ) {
      return yield* Effect.fail(
        new PreviewFailure({ operation: "Preview fork deployment forbidden" }),
      );
    }
    ...
    if (!/^[a-f0-9]{40}$/u.test(sha) || typeof baseRepository["id"] !== "number") {
      return yield* Effect.fail(
        new PreviewFailure({ operation: "Preview GitHub identity invalid" }),
      );
    }
...
export const resolvePreviewRun = (
  repository: string,
  runId: string,
  ...
  Effect.gen(function* () {
    if (!/^\d+$/u.test(runId)) {
      return yield* Effect.fail(new PreviewFailure({ operation: "Preview run ID invalid" }));
    }
    ...
    const expectedPath =
      operation === "deploy"
        ...
        : ".github/workflows/preview_cleanup_request.yml";
    ...
    if (
      ...
      return yield* Effect.fail(
        new PreviewFailure({ operation: "Preview build provenance rejected" }),
      );
    }
    ...
    if (!Array.isArray(pulls) || pulls.length !== 1) {
      return yield* Effect.fail(
        new PreviewFailure({
          operation: "Preview run has no unique PR; reconcile or dispatch explicitly",
        }),
      );
    }
    ...
    if (typeof pr !== "number") {
      return yield* Effect.fail(new PreviewFailure({ operation: "Preview run PR invalid" }));
    }
    ...
    if (operation === "deploy" && (pull.state !== "open" || pull.head !== run["head_sha"])) {
      return yield* Effect.fail(
        new PreviewFailure({ operation: "Preview build is stale or PR closed" }),
      );
    }

    if (operation === "destroy" && pull.state !== "closed") {
      return yield* Effect.fail(
        new PreviewFailure({ operation: "Preview PR reopened; cleanup skipped" }),
      );
    }
