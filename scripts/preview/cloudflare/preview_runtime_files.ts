import { readPreviewMigrationArtifact } from "../database/preview_migration_artifact.ts";
import { cp, lstat, mkdtemp, readdir, rm, writeFile } from "node:fs/promises";
import nodePath from "node:path";
import { Effect } from "effect";
import { derivePreviewSessionSecret, previewIo } from "../preview_model.ts";
import type { PreviewOwner } from "../preview_model.ts";

/** Keep rendered credentials scoped to deployment and remove them on failure or interruption. */
export const acquirePreviewRuntimeDirectory = Effect.acquireRelease(
  previewIo("Preview temporary directory failed", () => mkdtemp("/tmp/preview-deploy-")),
  (directory) =>
...
export const validatePreviewArtifacts = (root: string) =>
  previewIo("Preview artifact validation failed", async () => {
    const expected = ["api.mjs", "workflows.mjs", "frontend.mjs", "assets", "migrations.json"];
    ...
    Effect.andThen(() => readPreviewMigrationArtifact(root)),
    Effect.asVoid,
  );
