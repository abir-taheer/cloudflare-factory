...
export const reconcilePreviewEnvironments = (
  repository: string, owner: PreviewOwner, credentials: PreviewCredentials, cf: PreviewCloudflare, state: PreviewStateStore,
) => Effect.gen(function* () {
  const manifests = yield* state.list(owner);
  const failures: number[] = [];
  const recordFailure = () => Effect.sync(() => { failures.push(1); });
  for (const manifest of manifests.filter((item) => item.status !== "deleted")) {
    yield* Effect.gen(function* () {
      ...
      if (pull["state"] !== "open" && pull["state"] !== "closed") return yield* Effect.fail(new PreviewFailure({ operation: "Preview PR state unknown" }));
      if (pull["state"] === "closed" || Date.parse(manifest.expiresAt) <= Date.now() || manifest.status === "deleting") {
        yield* cleanupPreviewEnvironment(manifest, credentials, cf, state);
      }
      return yield* Effect.void;
    }).pipe(Effect.catchTag("PreviewFailure", recordFailure));
  }
  if (failures.length > 0) return yield* Effect.fail(new PreviewFailure({ operation: "Preview reconciliation incomplete; manifests retained" }));
  return yield* Effect.void;
});
