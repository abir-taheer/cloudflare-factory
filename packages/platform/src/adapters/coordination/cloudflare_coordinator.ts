...
import { Coordinator } from "../../capability_services.js";
import { capabilityOperation, validateLeaseInput } from "../capability_operation.js";

/** Minimal RPC namespace compatible with a deployed lease Durable Object. */
interface CoordinatorLeaseRpc {
...
export interface CoordinatorNamespace {
  getByName(name: string): CoordinatorLeaseRpc;
}

/** Durable Object adapter uses one named object per lease key. */
export const durableObjectCoordinatorLayer = (namespace: CoordinatorNamespace) =>
  Layer.succeed(
    Coordinator,
    Coordinator.of({
...
export class DurableObjectLeaseStorage {
  ...
  async acquire(token: string, ttlMs: number): Promise<boolean> {
    ...
    return this.state.storage.transaction(async (storage) => {
      const lease = await storage.get<CoordinatorLeaseRecord>("lease");
