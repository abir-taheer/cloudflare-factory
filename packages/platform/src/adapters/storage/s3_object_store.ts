...
export const s3ObjectStoreLayer = (client: S3Client, bucket: string) =>
  Layer.succeed(
    ObjectStore,
    ObjectStore.of({
      put: (key, body) =>
      ...
      get: (key) =>
        capabilityOperation("objectStore", "get", async () => {
          try {
            ...
            if (!object.Body) {
              throw new Error("S3 object response has no body");
            }
      ...
      delete: (key) =>
        capabilityOperation("objectStore", "delete", async () => {
