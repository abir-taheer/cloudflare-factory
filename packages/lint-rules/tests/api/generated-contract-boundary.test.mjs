...
test("generated declaration style exemption preserves neighboring authored declarations and security", () => {
  ...
  try {
    ...
    config.jsPlugins = [
      { name: "factory", specifier: path.resolve("packages/lint-rules/factory-lint-plugin.mjs") },
    ];
    ...
    const generatedFile = path.join(sourceDirectory, "api-paths.d.ts");
    ...
    const generatedDiagnostics = report.diagnostics.filter((entry) =>
      entry.filename.endsWith("/api-paths.d.ts"),
    );
