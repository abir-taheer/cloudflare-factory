...
const standardFiles = new Set(["AGENTS.md", "README.md", "Dockerfile", "LICENSE", "LICENSE.md"]);
...
function hasConventionalPathName(relativePath, directory) {
  ...
  return basename.split(".").every((segment) => kebabName.test(segment));
}
...
export async function findPathConventionViolations(root) {
  ...
  const visit = async (relativeDirectory) => {
    ...
    if (directFileCount > maximumDirectAuthoredFiles) {
      violations.push(
        `${relativeDirectory || "."}: directory contains ${directFileCount} direct authored files; maximum is ${maximumDirectAuthoredFiles}`,
      );
    }
    ...
    for (const entry of entries) {
      ...
      if (!hasConventionalPathName(relativePath, directory)) {
        violations.push(
          `${relativePath}: ${directory ? "directory" : "file"} name must use kebab-case`,
        );
      }
