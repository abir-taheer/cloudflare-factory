...
import { createApiApplication } from "../../../apps/api/src/http/api_application.js";
import { apiDocumentConfiguration } from "../../../apps/api/src/http/api_openapi.js";
...
try {
  ...
  const formatOptions = await resolveConfig(path.join(packageRoot, "src/api_paths.d.ts"));
  const typeText = await format(astToString(syntax), { ...formatOptions, parser: "typescript" });

  const artifacts = [
    ["openapi.json", documentText],
    ["src/api_paths.d.ts", typeText],
  ] as const;
